...
use App\Exports\Reportes\InfoAlumnosExportCredenciales;
...
class PlantelesFotosComponentCredenciales extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.reportes.planteles-fotos-component-credenciales');
    }
    ...
    public function mount(){
        ...
        $this->ciclo_activo = CicloEscModel::orderBy('per_inicio', 'desc')->first();
    ...
    public function generar_documento()
    {
        ...
        Excel::store(
            new InfoAlumnosExportCredenciales(
    ...
    private function alumnos_plantel($periodo, $grupo, $plantel, $ciclo){
        if ($periodo != "" && $grupo != 0) {
            ...
            ->selectRaw("alu_alumno.noexpediente, 
                MIN(alu_alumno.id) as id") // Seleccionar el ID mínimo para evitar duplicados
            ->groupBy('alu_alumno.noexpediente') // Agrupar por noexpediente para asegurar que cada expediente sea único
            ->orderBy('alu_alumno.noexpediente', 'asc') // Ordenar por noexpediente si es necesario
            ->get();
            ...
            ->where('esc_grupo.nombre', '!=', 'ActasExtemporaneas')
            ->selectRaw("alu_alumno.noexpediente, 
                MIN(alu_alumno.id) as id, 
                MIN(esc_grupo.periodo) as periodo, 
                MIN(esc_grupo.nombre) as grupo") // Seleccionar el valor mínimo para evitar duplicados
            ->groupBy('alu_alumno.noexpediente') // Agrupar por noexpediente para asegurar que cada expediente sea único
            ->orderBy('periodo', 'asc')
            ->orderBy('grupo', 'asc')
            ->get();
