...
class ParmateriasreprobadasComponent extends Component
{
    public $plantel;
    public $grupos ;
    public $docente;
    ...
    public function render()
    {
      ...
      $dat = DB::select('call pa_cursosreprobados (?,?,?,?,?)  ',array($cicloesc->id,$plantel_sel->id,"'".$this->grupos."'","'".$curso."'",$docente));
        $datos=collect($dat)->sortBy([['clave','asc'],['grupo','asc'],['turno','asc'],['alumno','asc']]);
      /*dd($cicloesc->id.'-'.$plantel_sel->id.'-'."'".$this->grupos."'".'-'."'".$curso."'".'-'.$docente);*/
        ...
        return view('livewire.reportes.parmateriasreprobadas-component',compact('datos' ,'cicloesc', 'plantelnom', 'titulo'));
